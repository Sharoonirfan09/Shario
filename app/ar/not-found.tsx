import type { Metadata } from "next";
import { Container, Heading, PillLink } from "@/components/ui";

export const metadata: Metadata = {
  title: "الصفحة غير موجودة",
  robots: { index: false, follow: true },
};

export default function ArabicNotFound() {
  return (
    <Container className="flex min-h-[60vh] flex-col items-center justify-center py-24 text-center">
      <p className="eyebrow flex items-center gap-3 text-carbon/55">
        <span aria-hidden="true" className="h-px w-6 bg-mist" />
        404
      </p>
      <Heading as="h1" scale="md" className="mt-5">
        هذه الصفحة انتقلت، أو لم تكن موجودة أصلاً.
      </Heading>
      <p className="mt-6 max-w-[480px] text-[1.0625rem] leading-[1.7] text-carbon/70">
        تحققوا من العنوان، أو ابدؤوا من أحد هذه الروابط بدلاً من ذلك.
      </p>
      <div className="mt-9 flex flex-wrap items-center justify-center gap-3.5">
        <PillLink href="/ar" tone="solid" size="lg">
          العودة للرئيسية
        </PillLink>
        <PillLink href="/ar/services" tone="outline" size="lg">
          خدماتنا
        </PillLink>
        <PillLink href="/ar/contact" tone="outline" size="lg">
          تواصل
        </PillLink>
      </div>
    </Container>
  );
}
